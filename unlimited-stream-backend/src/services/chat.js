...
function initChat(httpServer) {
  ...
  io.on('connection', (socket) => {
    socket.on('chat:join', async (channel) => {
      ...
      socket.join(room);
      // Any real logged-in account (admin/owner/channel) counts as staff for
    ...
    socket.on('chat:message', async (payload) => {
      ...
      const outgoing = { id: saved?._id, username, text, replyTo, ts: Date.now(), senderKey, senderType: isStaff ? 'staff' : 'student' };
