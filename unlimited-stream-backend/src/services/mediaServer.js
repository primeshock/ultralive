...
function createMediaServer() {
  ...
  nms.on('prePublish', async (id, StreamPath, args) => {
    ...
    const user = await User.findOne({ username, role: 'channel' });
  ...
  nms.on('postPublish', async (id, StreamPath) => {
    ...
    const user = await User.findOneAndUpdate({ username, role: 'channel' }, { isLive: true }, { new: true });
    if (!user) return;
  ...
  nms.on('donePublish', async (id, StreamPath) => {
    ...
    await User.findOneAndUpdate({ username, role: 'channel' }, { isLive: false });
